...
//! Write a ringbuffer data structure.
//!
...
impl<T> RingBuffer<T> {
    fn new(capacity: usize) -> Self {
        let mut data = Vec::<Option<T>>::with_capacity(capacity + 1);
    ...
    fn enqueue(&mut self, value: T) -> Option<T> {
        ...
        let previous = self.data.swap_remove(self.write);
        if previous.is_some() {
            self.read = (self.read + 1) % (self.data.capacity() - 1);
        }
        self.write = (self.write + 1) % (self.data.capacity() - 1);
        previous
    }
    ...
    fn dequeue(&mut self) -> Option<T> {
        ...
        self.data.push(None);
        let value = self.data.swap_remove(self.read);
        if value.is_none() {
            self.write = (self.write + 1) % (self.data.capacity() - 1);
        }
        self.read = (self.read + 1) % (self.data.capacity() - 1);
    ...
    fn peek(&self) -> Option<&T> {
        self.data.get(self.read)?.as_ref()
    }
    ...
    fn len(&self) -> usize {
        match self.read.cmp(&self.write) {
            ...
            Ordering::Equal => self.data.capacity() - 1,
            Ordering::Greater => self.data.capacity() - 1 - self.read + self.write,
        }
    }
...
/// (bonus): write a simple "DSL" (domain-specific language) that can
...
mod tests {
    ...
    fn iter_dequeue() {
        ...
        rb.dequeue();
        assert_eq!(rb.iter().collect::<Vec<_>>(), vec![&4, &5, &6]);
    }
