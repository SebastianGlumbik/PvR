//! Run this file with `cargo test --test 05_range`.

//! write a simple data structure called `Range1D`, which represents a range of
...
impl Range1D {
    fn new(start: u64, end: u64) -> Result<Self, &'static str> {
        if start > end {
            return Err("Start must not be larger than end");
        }
        Ok(Range1D { start, end })
    }
    ...
    fn intersect(&self, other: Range1D) -> Option<Self> {
        let start = self.start.max(other.start);
        let end = self.end.min(other.end);
        if start > end {
            return None;
        }
        Some(Range1D { start, end })
    }

    fn iter(&self) -> Range1DIterator {
        Range1DIterator {
            current: self.start,
            end: self.end,
        }
