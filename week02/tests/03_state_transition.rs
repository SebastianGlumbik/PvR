//! Run this file with `cargo test --test 03_state_transition`.
...
fn pc_transition(state: ComputerState, event: Event) -> ComputerState {
    match event {
        Event::TurnOn if state == ComputerState::Off => ComputerState::Running {
            uptime: 0,
            idle_time: 0,
        },
        Event::TurnOn => state,
        Event::TurnOff => ComputerState::Off,
        Event::PassTime(time) => match state {
            ComputerState::Running { idle_time, .. } if idle_time + time >= 1500 => {
                ComputerState::Off
            }
            ComputerState::Running { uptime, idle_time } if idle_time + time >= 1000 => {
                ComputerState::Sleeping {
                    uptime: uptime + time,
                    sleep_time: idle_time + time - 1000,
                }
            }
            ComputerState::Running { uptime, idle_time } => ComputerState::Running {
                uptime: uptime + time,
                idle_time: idle_time + time,
            },
            ComputerState::Sleeping { sleep_time, .. } if sleep_time + time >= 500 => {
                ComputerState::Off
            }
            ComputerState::Sleeping { uptime, sleep_time } => ComputerState::Sleeping {
                uptime: uptime + time,
                sleep_time: sleep_time + time,
            },
            ComputerState::Off => state,
        },
        Event::MoveMouse => match state {
            ComputerState::Running { uptime, .. } => ComputerState::Running {
                uptime,
                idle_time: 0,
            },
            ComputerState::Sleeping { uptime, .. } => ComputerState::Running {
                uptime,
                idle_time: 0,
            },
            ComputerState::Off => state,
