...
// Create an enum that represents three simple mathematical operations
...
fn perform_calculation(number: i32, op: Op) -> i32 {
    match op {
        Op::Add(x) => number + x,
        Op::Sub(x) => number - x,
        Op::Clamp { low, high } if low <= number && high >= number => number,
        Op::Clamp { low, high } => {
            if number < low {
                low
            } else {
