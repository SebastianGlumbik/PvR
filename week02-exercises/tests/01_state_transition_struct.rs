...
impl ComputerState {
    ...
    fn uptime(&self) -> u32 {
        self.uptime.unwrap_or_default()
    }
    fn idle_time(&self) -> u32 {
        self.idle_time
    }
    fn sleep_time(&self) -> u32 {
        self.sleep_time.unwrap_or_default()
    }
}
...
fn pc_transition(mut computer: ComputerState, event: Event) -> ComputerState {
    match event {
        Event::TurnOn if !computer.is_on() => {
            computer.uptime = Some(0);
        }
        Event::TurnOff => {
            computer = ComputerState::new_off();
        }
        Event::PassTime(time) if computer.is_on() => {
            computer.uptime = Some(computer.uptime() + time);

            if computer.is_sleeping() {
                computer.sleep_time = Some(computer.sleep_time() + time);
            } else {
                computer.idle_time = computer.idle_time() + time;
            }

            if computer.idle_time() > 1000 {
                computer.sleep_time = Some(computer.idle_time() - 1000);
