...
// Write a BLAZINGLY FAST, high-performance zero-copy parser of... *checks notes* cinema tickets?
//
...
impl Day {
    fn parse(day: &str) -> Result<Day, &str> {
...
fn check_string(string: Option<&str>) -> Option<&str> {
    string.filter(|s| {
        s.chars()
            .all(|c| c.is_ascii_alphabetic() || c.is_ascii_digit() || c.is_ascii_whitespace())
    })
}

fn parse_ticket(text: &str) -> Option<Ticket> {
    let mut split = text.split(";");
    let ticket = Ticket {
        movie: check_string(split.next())?,
        day: Day::parse(split.next()?).ok()?,
        visitor: match split.next() {
            None => None,
            v => Some(check_string(v)?).filter(|x| x.is_empty().not()),
        },
    };
