// Implement storage of data in both encrypted and unencrypted forms.
...
impl Data {
    ...
    pub fn read(&mut self) -> Option<u8> {
        self.data.get(self.index).map(|byte| {
            self.index += 1;
            *byte
        })
    }
    pub fn encrypt(self, key: u8) -> DataEncrypted {
        DataEncrypted { data: self, key }
    }
}
...
impl DataEncrypted {
    pub fn read(&mut self) -> Option<u8> {
        self.data.read().map(|byte| byte.bitxor(self.key))
    }
