    ...
    let server = std::thread::spawn({
        ...
        move || {
            ...
            for stream in listener.incoming() {
                ...
                if end_flag.load(std::sync::atomic::Ordering::Relaxed) {
                ...
                connections = connections
                    .into_iter()
                    .filter_map(|connection: JoinHandle<()>| {
                        if connection.is_finished() {
                            connection.join().unwrap_or_default();
                            None
                        } else {
                            Some(connection)
                        }
                ...
                if connections.len() >= opts.max_clients {
                    client.disconnect(Some(ServerToClientMsg::Error("Server is full".to_string())));
                    continue;
                }
            ...
            for (_, client) in clients.lock().unwrap().drain() {
                client.disconnect(None);
            }
...
mod tests {
    ...
    use crate::{run_server, RunningServer, ServerOpts, SocketWrapper};
