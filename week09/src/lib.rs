#![allow(unused)]
//! implement a simple chat server using non-blocking I/O
//!
...
//! non-blocking writes.
//!
//! (question): try to examine the message protocol from the last week. What issue would there be
...
impl Drop for RunningServer {
    fn drop(&mut self) {
        self.end.send(()).unwrap_or_default();
