...
//! implement a simple parallel job queue
//!
...
//!
//! (question): is it possible to enqueue work to WorkerQueue from multiple threads?
//! Try it and see what happens. If it's not possible, how could you make it work?
//! Answer: It is not possible to enqueue work from multiple threads, due to results not being safe to send across threads. We would need to use a Mutex to make it work.
//!
...
struct WorkerQueue<T: Send + 'static> {
    workers: Vec<(std::thread::JoinHandle<()>, SyncSender<Job<T>>)>,
    results: std::sync::mpsc::Receiver<T>,
    counter: Mutex<usize>,
}

impl<T: Send + 'static> WorkerQueue<T> {
    fn new(worker_size: usize, queue_size: usize) -> Self {
        let (result_sender, results) = std::sync::mpsc::sync_channel::<T>(worker_size * queue_size);
        let mut workers = Vec::with_capacity(worker_size);
        for _ in 0..worker_size {
            let result_sender = result_sender.clone();
            let (sender, receiver) = std::sync::mpsc::sync_channel::<Job<T>>(queue_size);
    ...
    {
        let mut counter = self.counter.lock().expect("Mutex lock failed");
        let (_, sender) = self.workers.get(*counter).expect("Out of bounds");
        *counter = (*counter + 1) % self.workers.len();
        sender.send(Box::new(f)).expect("Send failed");
    }
    fn next_result(&self) -> T {
        self.results.recv().expect("Receive failed")
    }
    fn close(self) {
        drop(self.results);
        for (handle, sender) in self.workers {
            drop(sender);
