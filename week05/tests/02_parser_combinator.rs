...
//! Implement a JSON parser using parser combinators.
...
fn take_while<F: FnMut(char) -> bool>(mut f: F) -> impl FnMut(&str) -> ParseResult<&str, String> {
    move |input| {
...
fn string_parser(string: &str) -> impl FnMut(&str) -> ParseResult<&str, String> + '_ {
    move |input| match input.strip_prefix(string) {
        Some(rest) => Ok((rest, string.to_string())),
        None => Err(format!("Input does not begin with {}", string)),
    }
}

fn map<F, T, B>(mut parser: impl Parser<T>, mut f: F) -> impl FnMut(&str) -> ParseResult<&str, B>
where
    F: FnMut(T) -> B,
{
...
fn try_map<F, T, B>(
    ...
    mut f: F,
) -> impl FnMut(&str) -> ParseResult<&str, B>
where
    F: FnMut(T) -> Result<B, String>,
{
    move |input| match parser.parse(input) {
        Ok((rest, value)) => match f(value) {
...
fn or<T>(
    mut p1: impl Parser<T>,
    mut p2: impl Parser<T>,
) -> impl FnMut(&str) -> ParseResult<&str, T> {
    move |input| match (p1.parse(input), p2.parse(input)) {
        (Ok(inside), _) => Ok(inside),
        (_, Ok(inside)) => Ok(inside),
        (Err(a), Err(b)) => Err(format!("Both parser failed: {} and {}", a, b)),
    }
}

fn opt<P, T>(mut parser: P) -> impl FnMut(&str) -> ParseResult<&str, Option<T>>
where
    P: FnMut(&str) -> ParseResult<&str, T>,
{
...
fn preceded_by<T, U>(
    mut p1: impl Parser<T>,
    mut p2: impl Parser<U>,
) -> impl FnMut(&str) -> ParseResult<&str, U> {
    move |input| match p1.parse(input) {
        Ok((rest, _)) => p2.parse(rest),
        Err(e) => Err(e),
    }
}
...
fn followed_by<T, U>(
    ...
    move |input| match p1.parse(input) {
        Ok((rest, value)) => match p2.parse(rest) {
...
fn delimited_by<T>(
    a: char,
    b: char,
    mut parser: impl Parser<T> + for<'a> FnMut(&'a str) -> ParseResult<&str, T>,
) -> impl FnMut(&str) -> ParseResult<&str, T> {
    move |input| followed_by(preceded_by(char_parser(a), &mut parser), char_parser(b))(input)
}

fn sequence<T, U>(
    mut p1: impl Parser<T>,
    mut p2: impl Parser<U>,
) -> impl FnMut(&str) -> ParseResult<&str, (T, U)> {
    move |input| match p1.parse(input) {
        Ok((rest, value1)) => match p2.parse(rest) {
            Ok((rest, value2)) => Ok((rest, (value1, value2))),
            Err(e) => Err(e),
        },
        Err(e) => Err(e),
    }
...
fn repeated<T>(mut parser: impl Parser<T>) -> impl FnMut(&str) -> ParseResult<&str, Vec<T>> {
    move |input| {
        let mut result = Vec::new();
        let mut rest = input;
        while let Ok((new_rest, value)) = parser.parse(rest) {
            result.push(value);
            rest = new_rest;
        }
        Ok((rest, result))
...
fn tag_parser(tag: &str) -> impl FnMut(&str) -> ParseResult<&str, String> + '_ {
    move |input| {
        let (rest, _) = string_parser(&format!("<{}>", tag))(input)?;
        let mut result = String::new();
        for (index, c) in rest.char_indices() {
            if c == '<' {
                let (rest, _) = string_parser(&format!("</{}>", tag))(&rest[index..])?;
                return Ok((rest, result));
            }
            result.push(c);
        }
        Err("Tag not closed".to_string())
    }
}
